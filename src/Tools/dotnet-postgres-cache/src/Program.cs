...
public class Program
{
    ...
    public int Run(string[] args)
    {
        ...
        try
        {
            ...
            {
                Name = "dotnet postgres-cache",
                FullName = "Postgres Cache Command Line Tool",
                Description =
                    "Creates table and indexes in Postgres database to be used for distributed caching",
            };
    ...
    private int CreateTableAndIndexes(IReporter reporter)
    {
        ...
        using (var connection = new NpgsqlConnection(_connectionString))
        {
            ...
            var sqlQueries = CreateSqlQueries();
            var command = new NpgsqlCommand(sqlQueries.TableInfo, connection);
            ...
            using (var transaction = connection.BeginTransaction())
            {
                try
                {
                    command = new NpgsqlCommand(sqlQueries.CreateTable, connection, transaction);
    ...
    private void ValidateConnectionString()
    {
        ...
        catch (Exception ex)
        {
            throw new ArgumentException(
                $"Invalid Postgres connection string '{_connectionString}'. {ex.Message}", ex);
        }
    }
}
